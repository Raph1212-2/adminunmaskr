...
// ─── HELPERS ──────────────────────────────────────────────────────────────────
// Escapes user-entered text before it goes into an email's HTML. Without
// this, someone could put something like <script> or <img onerror=...> in
// their name, bank name, or a message — and it would get sent as literal,
// executable HTML to whoever opens that email (a classic stored-XSS bug).
...
const Users = () => {
  ...
  const fetchUsers = async () => {
    ...
    // Batched instead of one query per user: 3 queries total no matter how
    // many users there are, then aggregate client-side. The old version did
    // 3 queries PER user (300 queries for 100 users) — this scales much
    // better as your user base grows.
    ...
    const [{ data: allMessages }, { data: spentRows }, { data: earnedRows }] = await Promise.all([
      supabase.from("messages").select("recipient_id"),
      supabase.from("transactions").select("user_id, amount").eq("type", "hint_purchase").eq("status", "completed"),
      supabase.from("transactions").select("user_id, amount").eq("type", "hint_earning").eq("status", "completed"),
    ]);

    const messageCountByUser = {};
    (allMessages || []).forEach(m => { messageCountByUser[m.recipient_id] = (messageCountByUser[m.recipient_id] || 0) + 1; });
    const spentByUser = {};
    (spentRows || []).forEach(r => { spentByUser[r.user_id] = (spentByUser[r.user_id] || 0) + Number(r.amount); });
    const earnedByUser = {};
    (earnedRows || []).forEach(r => { earnedByUser[r.user_id] = (earnedByUser[r.user_id] || 0) + Number(r.amount); });
    ...
    const enriched = profilesData.map(u => ({
      ...
      messages: messageCountByUser[u.id] || 0,
      spent: `₦${(spentByUser[u.id] || 0).toLocaleString()}`,
      earned: `₦${(earnedByUser[u.id] || 0).toLocaleString()}`,
    ...
    if (dep.email) {
      await supabase.functions.invoke("send-email", {
        body: {
          ...
          htmlContent: `<p>Hi ${escapeHtml(dep.name)},</p><p>Your deposit of ${escapeHtml(dep.amount)} has been confirmed and added to your Unmaskr wallet.</p>`
        }
      });
    }
      ...
      if (w.email) {
        await supabase.functions.invoke("send-email", {
          body: {
            ...
            htmlContent: `<p>Hi ${escapeHtml(w.name)},</p><p>Your withdrawal of ${escapeHtml(w.amount)} has been sent to your ${escapeHtml(w.bank)} account ending in ${escapeHtml(w.account.slice(-4))}.</p>`
          }
        });
      }
...
const Complaints = () => {
  ...
  const sendReply = async () => {
    ...
    if (selected.email) {
      await supabase.functions.invoke("send-email", {
        body: { to: selected.email, toName: selected.user, subject: `Re: ${escapeHtml(selected.subject)}`, htmlContent: `<p>${escapeHtml(replyText)}</p>` }
      });
    }
